...
func getAccountName(w http.ResponseWriter, r *http.Request) {
	...
	err = db.Select(&results, "SELECT p.`id`, p.`user_id`, p.`body`, p.`mime`, p.`created_at` FROM `posts` AS p JOIN `users` AS u ON p.user_id = u.id WHERE u.`del_flg` = 0 AND `user_id` = ? ORDER BY p.`created_at` DESC LIMIT 20", user.ID)
	if err != nil {
		log.Print(err)
		return
	}
...
func getPostsID(w http.ResponseWriter, r *http.Request) {
	...
	// err = db.Select(&results, "SELECT * FROM `posts` AS p JOIN `users` AS u ON p.user_id = u.id WHERE u.`del_flg` = 0 AND p.`id` = ?", pid)
	if err != nil {
		log.Print(err)
		return
	}
